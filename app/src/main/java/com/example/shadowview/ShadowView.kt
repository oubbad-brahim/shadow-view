...
import com.example.shadowview.Utils.OnUpdate
...
class ShadowView @JvmOverloads constructor(
    ...
    private fun getScript(): Pair<ScriptIntrinsicBlur, RenderScript> {
        ...
        val element =
            if (shadow_with_color) Element.U8_4(renderScript) else Element.U8(renderScript)
    ...
    init {
        if (!isInEditMode) {
            ...
            shadow_color = attributes.getColor(R.styleable.ShadowView_shadow_color, 51 shl 24)
            shadow_with_color =
                attributes.getBoolean(R.styleable.ShadowView_shadow_with_color, false)
            shadow_with_content =
                attributes.getBoolean(R.styleable.ShadowView_shadow_with_content, true)
            shadow_with_dpi_scale =
                attributes.getBoolean(R.styleable.ShadowView_shadow_with_dpi_scale, true)
            shadow_with_css_scale =
                attributes.getBoolean(R.styleable.ShadowView_shadow_with_css_scale, true)
            shadow_x_shift = attributes.getDimension(R.styleable.ShadowView_shadow_x_shift, 0f)
            shadow_y_shift = attributes.getDimension(R.styleable.ShadowView_shadow_y_shift, 0f)
            shadow_downscale = attributes.getFloat(R.styleable.ShadowView_shadow_downscale, 1f)
            shadow_radius = attributes.getFloat(R.styleable.ShadowView_shadow_radius, 6f)
            attributes.recycle()
        }
    ...
    private inline val blurSMatrix: Matrix // to draw inside the small blurBitmap
        get() = scaleMatrix(
            ratioPixelsToDp / shadow_downscale / cssRatio,
            ratioPixelsToDp / shadow_downscale / cssRatio
        )
